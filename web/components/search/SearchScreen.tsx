...
// `applied` echo (INTERACTION_STATES §6/§7). RESPONSIVE: mobile keeps the search
// app-bar; desktop renders the shared top nav (WebChrome) + a wide results grid.
...
export default function SearchScreen() {
      ...
      <div className="pd-root pd-web" data-pd-theme="a">
        ...
        <main className="pd-scroll pdw-scroll">
          <div className="pdw-detailwrap">
            <h1 className="pdw-h1" style={{ marginBottom: 16 }}>Поиск букетов</h1>
            <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>{searchField}</div>
            <div className="pd-chiprow" style={{ marginBottom: 20 }}>{sizeChips}</div>
            {results('pdw-grid')}
          </div>
        </main>
      </div>
  ...
  return (
    <div className="pd-root" data-pd-theme="a">
      ...
      <main className="pd-scroll">{results('pd-grid')}</main>
    </div>
  );
}
