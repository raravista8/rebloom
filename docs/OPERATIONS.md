...
- **Visual testing has two layers** (`web/tests/visual/`, two Playwright projects `mobile-360` + `desktop-1280`): (1) DOM/functional specs + deterministic desktop **geometry guards** (`desktop.spec.ts`: no-clip via `scrollWidth≤clientWidth`, split spans full width, no horizontal overflow) — these catch structural breakage and never flake; (2) **pixel-diff** (`pixel.spec.ts`, `toHaveScreenshot` ≤2%, animations disabled + `emulateMedia({reducedMotion:'reduce'})`). Functional specs run in `mobile-360`; `desktop.spec.ts` is desktop-only; `pixel.spec.ts` runs in both. Reach for the geometry guard first — it's deterministic.
