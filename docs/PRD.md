...
> ⚠️ **ОБНОВЛЕНО — ADR-0013 (запуск без эскроу, «оплата при встрече»).** MVP запускается **без эскроу и онлайн-оплаты**: площадка не проводит, не держит и не возвращает деньги — люди договариваются в чате и платят **при встрече** (наличные/перевод). Стейт-машина сделки: `agreed → meeting → done` (+ `problem`, `cancelled`). Для MVP: **FR-021/022/023/025/026/027** (эскроу/оплата/таймаут/webhook/релиз/комиссия) переописаны как «оплата при встрече» (создание сделки открывает чат; `confirm-receipt` → `done`; споры → жалобы `problem`; отмена → `cancelled`); **NFR-compliance** — PCI/54-ФЗ-через-агента/ФЗ-115 сняты со скоупа MVP (ФЗ-152 без изменений); **North-star GMV/комиссия** — на паузе, монетизация = отдельный ADR post-MVP. Где текст ниже говорит «эскроу/оплата/комиссия» — для MVP читать по ADR-0013.
