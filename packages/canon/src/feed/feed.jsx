...
// l может быть lineId ('1'/'sp2' → цвет из PD_METRO_LINES) или уже цветом ('#RRGGBB',
// когда станции приходят data-driven пропсом stations с lines:[{name,color}]).
  ...
  return (
    <span className="pd-mdots" aria-hidden="true">
      {ls.slice(0, 3).map((l, i) => (
        <i key={i} style={{ background: (typeof l === 'string' && l[0] === '#') ? l : (PD_METRO_LINES[l] || '#A1A2A3'), width: size, height: size }} />
      ))}
    </span>
  );
}
